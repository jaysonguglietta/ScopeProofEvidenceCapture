...
: "${SCOPEPROOF_HOSTED_API_ORIGIN:?Exact HTTPS hosted API origin is required}"
...
[[ "$SCOPEPROOF_HOSTED_API_ORIGIN" =~ '^https://[A-Za-z0-9.-]+$' && "$SCOPEPROOF_HOSTED_API_ORIGIN" != *".."* ]] || { echo "Hosted API origin must be an exact HTTPS hostname with no path, port, query, or fragment." >&2; exit 1; }
...
plutil -replace ScopeproofHostedAPIOrigins -xml "<array><string>$SCOPEPROOF_HOSTED_API_ORIGIN</string></array>" "$plist"
plutil -lint "$plist"
