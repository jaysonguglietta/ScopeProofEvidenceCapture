...
export function validateTenantSqlOptions(candidate) {
  ...
  if (!new RegExp(`^arn:(?:aws|aws-us-gov|aws-cn):kms:${escapedRegion}:${escapedAccount}:key/[0-9a-f-]{36}$`).test(value.kmsKeyArn)) throw new Error("--kms-key-arn must be a customer-managed key ARN in the configured account and region.");
  return Object.freeze(value);
}

export async function renderTenantSql(candidate) {
  const options = validateTenantSqlOptions(candidate);
  const [schema, grants] = await Promise.all([
  ...
  const seed = [
    ...
    `VALUES (${sqlLiteral(options.tenantId)}, ${sqlLiteral(options.slug)}, ${sqlLiteral(options.displayName)}, 'PROVISIONING', ${sqlLiteral(options.hostname)}, ${options.retentionDays}, ${sqlLiteral(options.retentionMode)}, ${sqlLiteral(options.awsAccountId)}, ${sqlLiteral(options.awsRegion)}, ${sqlLiteral(options.quarantineBucket)}, ${sqlLiteral(options.evidenceBucket)}, ${sqlLiteral(options.kmsKeyArn)});`,
...
function parseArguments(argv) {
  const aliases = new Map([
    ...
    ["retention-mode", "retentionMode"], ["runtime-role", "runtimeRole"], ["output", "output"],
