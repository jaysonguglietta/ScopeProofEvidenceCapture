...
export async function POST(request: Request) {
  try {
    ...
    return Response.json({ package: await buildAssessorPackage(user, String(body.assessmentId || "")) }, { status: 201 });
  } catch (error) { return jsonError(error); }
}
