...
enum LocalConsoleAssets {
    ...
    <body>
      ...
      <div class="app-shell">
        <aside class="sidebar">
          ...
          <div class="local-card"><span class="status-dot"></span><div><strong>Local mode</strong><small>Private to this Mac</small></div></div>
        ...
        <main id="main">
          ...
          <section id="overview-view" class="view active" aria-labelledby="overview-heading">
            <div class="hero">
              <div><span class="pill success">Loopback protected</span><h2 id="overview-heading">Your evidence stays on this Mac.</h2><p>Search, review, and prepare locally captured evidence without a hosted login or device-enrollment token. Hosted synchronization remains optional.</p></div>
          ...
          <section id="evidence-view" class="view" aria-labelledby="evidence-heading">
            <div class="section-intro"><div><span class="eyebrow">Auditor-ready organization</span><h2 id="evidence-heading">Evidence library</h2><p>Filter by framework, control, lifecycle state, asset, owner, evidence ID, or Jira issue.</p></div><span id="result-count" class="result-count">0 items</span></div>
            <form class="filter-bar" id="filters" role="search">
              ...
              <label><span>Status</span><select id="review-status"><option value="">All statuses</option><option>Draft</option><option>In Review</option><option>Approved</option><option>Rejected</option><option>Superseded</option></select></label>
              <button class="button secondary" type="button" id="clear-filters">Clear</button>
            </form>
            <div id="evidence-grid" class="evidence-grid"></div>
            <div id="empty-state" class="empty hidden"><div>▱</div><h3>No matching evidence</h3><p>Adjust the filters or capture new evidence from the Scopeproof menu.</p><button class="button primary" type="button" id="empty-capture">Capture evidence</button></div>
          </section>
          ...
          <section id="help-view" class="view" aria-labelledby="help-heading">
            ...
            <div class="help-grid">
              ...
              <article class="help-card"><span>2</span><h3>Capture and redact</h3><p>Scopeproof captures only after an explicit action, scans the image locally, masks sensitive values, adds the visible evidence banner, and asks you to review the exact pixels.</p></article>
              <article class="help-card"><span>3</span><h3>Review locally</h3><p>Use this console to search and move artifacts through Draft, In Review, Approved, Rejected, or Superseded. Every change is hash-chained.</p></article>
    ...
    (() => {
      ...
      function renderMetrics(summary) {
        const metrics = [
          ['Total evidence', summary.total], ['Approved', summary.approved], ['Needs review', summary.needsReview],
          ['Hosted receipts', summary.uploaded], ['Integrity alerts', summary.integrityFailures]
        ];
      ...
      function evidenceCard(item) {
        ...
        const image = node('img'); image.loading='lazy'; image.alt=`Screenshot preview for ${item.title}`; image.src=`/api/evidence/${encodeURIComponent(item.evidenceID)}/image`;
        const body=node('div','card-body'); const kicker=node('div','card-kicker');
        kicker.append(node('span',`pill ${pillClass(item.reviewStatus)}`,item.reviewStatus),node('small','',item.localTimestamp));
        const title=node('h3','',item.title); const meta=node('p','',`${item.complianceArea} · ${item.controlID} · ${item.system}`);
        const period=node('p','',`${item.environment} · ${item.assessmentPeriod}${item.jiraIssueKey ? ` · ${item.jiraIssueKey}` : ''}`);
        ...
        const reveal=node('button','button secondary','Reveal'); reveal.type='button'; reveal.addEventListener('click',()=>performAction('/api/actions/reveal',{evidenceID:item.evidenceID},'Evidence revealed in Finder.'));
        const review=node('button','button primary','Review'); review.type='button'; review.disabled=!item.lifecycleValid; review.addEventListener('click',()=>openReview(item));
        actions.append(reveal,review); body.append(kicker,title,meta,period,tags,actions); card.append(image,body); return card;
      }

      function renderEvidence(items) {
        state.evidence=items; $('result-count').textContent=`${items.length} ${items.length===1?'item':'items'}`;
        $('evidence-grid').replaceChildren(...items.map(evidenceCard)); $('empty-state').classList.toggle('hidden',items.length>0);
        const recent=items.slice(0,5).map((item)=>{ const row=node('div','recent-row'); const image=node('img'); image.alt=''; image.src=`/api/evidence/${encodeURIComponent(item.evidenceID)}/image`; const copy=node('div'); copy.append(node('strong','',item.title),node('small','',`${item.complianceArea} ${item.controlID} · ${item.localTimestamp}`)); row.append(image,copy,node('span',`pill ${pillClass(item.reviewStatus)}`,item.reviewStatus)); return row; });
        $('recent-evidence').replaceChildren(...recent);
        if (!recent.length) { const empty=node('p','', 'No local captures yet. Use Capture evidence to create the first artifact.'); $('recent-evidence').replaceChildren(empty); }
      }

      async function loadEvidence() {
        const params=new URLSearchParams();
        [['search','search'],['framework','complianceArea'],['control','controlID'],['review-status','reviewStatus']].forEach(([id,key])=>{ const value=$(id).value.trim(); if(value) params.set(key,value); });
        const payload=await request(`/api/evidence?${params}`); renderEvidence(payload.evidence);
        populateOptions('framework',payload.facets.frameworks,'All frameworks'); populateOptions('control',payload.facets.controls,'All controls');
      }
      ...
      $('filters').addEventListener('input',()=>{ window.clearTimeout(loadEvidence.timer); loadEvidence.timer=window.setTimeout(()=>loadEvidence().catch((error)=>showNotice(error.message,true)),180); });
      $('clear-filters').addEventListener('click',()=>{ $('filters').reset(); loadEvidence().catch((error)=>showNotice(error.message,true)); });
