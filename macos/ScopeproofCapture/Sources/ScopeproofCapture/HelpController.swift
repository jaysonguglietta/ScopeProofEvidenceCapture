...
final class HelpController: NSObject, NSWindowDelegate {
    ...
    private func helpText(outputDirectory: URL) -> NSAttributedString {
        ...
        add("1. Open Capture & Jira Settings. Enter the HTTPS Jira site, default project key, preferred attachment set, and any organization-specific handling instructions. Scopeproof does not request or store Jira credentials.\n2. Enter the destination issue key (for example, GRC-123) while classifying a capture. The key appears in the filename, screenshot banner, immutable manifest, search result, and assessor index.\n3. Review the evidence and mark it Approved before treating it as audit-ready. In Search Evidence, select the item and choose Copy Jira Comment. Paste the generated evidence summary, SHA-256, and attachment checklist into the intended ticket.\n4. Attach either the complete evidence set (PNG, capture manifest, review lifecycle, and signed receipt when available) or the approved assessor ZIP plus its separate checksum. Do not attach only the screenshot when the manifest and review trail are available.\n5. Download the attachment from Jira and verify its SHA-256. Confirm the Jira project permissions, external-auditor access, classification, and retention policy before upload. Never attach an unredacted source image, password, cookie, token, private key, or PAN.\n\n", size: 14)
        ...
        add("In the Scopeproof web console, open Connections and enroll a Mac capture device. Copy the one-time token into Capture & Jira Settings. The token is stored in your login Keychain, not preferences. Turn on Upload after capture when you are ready. Failed or offline uploads remain local and can be retried from the menu.\n\n", size: 14)
