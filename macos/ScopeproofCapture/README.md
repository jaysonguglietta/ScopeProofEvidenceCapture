...
For a production release, provision the offline update public key and validity window under `ScopeproofUpdatePublicKeys` and one exact HTTPS `ScopeproofUpdateDownloadOrigin` under `Info.plist`, then create the signed/notarized candidate with the protected production workflow. Download its exact attested seven-file artifact set and pass its directory, repository, approved commit, update key, version, sequence, exact versioned URL, team, and designated requirement to `./Scripts/publish_release.sh`. The script snapshots and verifies rather than rebuilds the candidate and signs an update envelope over the exact ZIP. Configure that envelope through `MACOS_RELEASE_MANIFEST_JSON`, `MACOS_RELEASE_SIGNATURE_DER_BASE64`, and an exact `MACOS_RELEASE_ALLOWED_HOSTS` value. The app derives `/macos/<version>/Scopeproof-Capture-<version>.zip` from its compiled origin instead of following a manifest-selected host or redirect, verifies the downloaded bundle ID/version, and stores the last accepted `(sequence, version, SHA-256)` tuple in a device-only Keychain item. It rejects rollback, same-sequence equivocation, wrong keys/digests/teams/requirements, Gatekeeper failures, and releases without a valid stapled notarization ticket.
