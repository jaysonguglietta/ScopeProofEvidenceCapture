...
1. Reject every manifest except the current hosted schema; validate bounded canonical fields, tenant/workspace, PNG structure/dimensions, filenames/content types, image and safety digests, enrollment-token HMAC, P-256 provenance signature/key ID, local event hash, capture-session ownership/scope, and exactly one active assessment with an explicit non-empty versioned catalog/system/control scope.
