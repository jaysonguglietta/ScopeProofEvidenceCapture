...
Repository SBOM generation requires `GITHUB_ORG` and `GITHUB_TOKEN` in the hosted Sites environment. Use a GitHub App installation token or fine-grained token restricted to the intended repositories with only **Metadata: read** and **Contents: read**. Operators then use **SBOMs → Generate SBOM** in the hosted web console; this capability is not part of the local-only Mac console. See the [repository SBOM guide](docs/SBOM_GUIDE.md) for assessment scope, supported lockfiles, approval, and auditor interpretation.
