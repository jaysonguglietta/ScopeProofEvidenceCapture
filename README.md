...
New native evidence is stored under `~/Documents/Scopeproof Evidence`. Captures created by earlier versions under `~/Pictures/Scopeproof Evidence` remain searchable, reviewable, uploadable, subject to configured retention, and exportable without an automatic move. Confirm that Documents-folder access and any iCloud, enterprise-sync, backup, and DLP behavior are approved for the evidence classification.
