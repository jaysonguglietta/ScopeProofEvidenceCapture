...
> commit `8cd2d5c`. It does not contain the current **Unreleased 1.10.0 (build 25)** native changes
> described elsewhere in this guide. No corresponding DMG has been published. Use this
> DMG only to evaluate the tagged build. It writes new captures under
...
> published. Current source is version `1.10.0` build `25`; no 1.10.0 artifact
> exists.
...
test. Do not assume a branch name proves that it contains them. Record the exact commit and require a clean
...
Source metadata or a successful build does not prove which copy is installed or running. After installation, verify the intended bundle directly:

```bash
/usr/libexec/PlistBuddy -c 'Print :CFBundleShortVersionString' "$HOME/Applications/Scopeproof Capture.app/Contents/Info.plist"
/usr/libexec/PlistBuddy -c 'Print :CFBundleVersion' "$HOME/Applications/Scopeproof Capture.app/Contents/Info.plist"
shasum -a 256 "$HOME/Applications/Scopeproof Capture.app/Contents/MacOS/ScopeproofCapture"
```

Record those values with the exact source commit used for the build. If the app was installed from the older DMG, inspect `/Applications/Scopeproof Capture.app` instead. Do not infer the installed version from this guide's Unreleased heading.

Scopeproof appears as a shield in the menu bar and opens its Local Console in the default browser. The console is served by the running Mac app on a random `127.0.0.1` port and is not published to the LAN or internet. A one-time nonce in the URL fragment is exchanged for a separate short-lived in-memory bearer and then cleared; no localhost authentication cookie is created. Do not copy or share that launch URL. Its Evidence library shows only the active tenant/workspace's local screenshots and, when the matching S3 destination is configured and verified, matching S3 screenshots with explicit storage badges and on-demand previews. If the tab is closed, choose **Open Local Console** from the shield menu or press `Command-Shift-L`.
...
Open **Capture & Jira Settings…** from the shield menu. Leave **Server URL** blank to prevent hosted synchronization. The Local Console, capture, search, review, Jira comment generation, and local assessor-package export continue to work even when no hosted OCR/DLP scanner or trusted timestamp authority exists. New captures use a signed current manifest and the selected tenant/workspace's locally anchored P-256 provenance identity. Trust-bearing lifecycle and local legal-hold actions require macOS local-user authentication and use independent signing/rollback domains.

The default `local/default` identity stores evidence under `~/Documents/Scopeproof Evidence/<framework>/<control>/`. A named tenant/workspace stores it under `~/Documents/Scopeproof Evidence/tenants/<tenant>/workspaces/<workspace>/<framework>/<control>/`. Configure that identity before enrolling the Mac, verifying S3, or capturing; changing it invalidates the prior Local Console session and requires matching hosted/S3 credentials. Existing captures under `~/Pictures/Scopeproof Evidence` remain discoverable; Scopeproof does not move or delete them automatically. Storage location does not determine trust. Current schema-8 evidence and signed schema-7 evidence may be locally trust-bearing under their respective lifecycle rules; schema 7 cannot cross the current hosted tenant/workspace boundary. Unsigned schema-6 and older artifacts are an explicit, visibly unverified browsing/migration path and are blocked from approval, upload, legal-hold/retention assertions, package export, and Jira trust workflows. Recapture unsigned legacy evidence with the current app rather than editing its manifest or lifecycle record. The Local Console's SQLite search index lives in the current user's Application Support folder and can be rebuilt from the immutable evidence manifests and lifecycle sidecars. A tenant-bound Keychain commit journal reconciles complete capture files with the monotonic capture head after an interrupted save.

## Optional hosted synchronization requirements

Hosted synchronization is not required for installation or local use. The checked-in `Info.plist` has an empty `ScopeproofHostedAPIOrigins` array, so an ordinary current-source release build rejects every remote HTTPS Server URL and remains local-only. A release operator must first compile one reviewed, exact, pathless HTTPS origin with `SCOPEPROOF_HOSTED_API_ORIGIN` through `Scripts/configure_macos_release_identity.sh`; never add a personal or historical service as a source default. When an administrator enrolls the Mac for that compiled origin, paste the one-time audience-bound device token and the same HTTPS Server URL under **Capture & Jira Settings…**. The token is stored in a device-only Keychain item; the schema-8 provenance signing key is a separate device-only P-256 Keychain identity. Set the Mac customer/workspace to the server's exact `LEGACY_TENANT_ID`/`LEGACY_WORKSPACE_ID` before capturing. Development-only loopback HTTP is accepted only by debug builds for local integration testing.

A production server accepts the current signed schema-8 PNG/manifest pair only and requires its tenant/workspace to match the server's isolated legacy binding. It independently rescans the exact PNG through the OCR/DLP service configured by the legacy-named `BROWSER_OCR_ENDPOINT`, `BROWSER_OCR_TOKEN`, and `BROWSER_OCR_ALLOWED_HOSTS` settings, verifies a trusted RFC 3161 timestamp, and finalizes the exact artifact into the server-side device chain. Recognized OCR text is evaluated transiently and is not retained. Until the binding, scan, timestamp, and final chain linkage succeed, the hosted item is quarantined and cannot be read, previewed, approved, packaged, exported, or sent to Jira.

If scanner or timestamp verification is unavailable, the hosted upload fails closed while the local artifact remains usable. Retry the unchanged current pair after recovery. An unsigned legacy capture must be recaptured; an older hosted item missing the independent scan or final linkage must be re-uploaded/rescanned. Administrators must never grandfather either population with direct database updates. Server operators must apply migrations through `0028_native_reconciliation_cursor.sql` and confirm production readiness has no scanner or trusted-timestamp failures before enabling native uploads. No AWS deployment is performed by installing or building the Mac app.
...
The development-preview DMG does not use the trusted production updater. In a production build configured for the organization's exact download and hosted API origins, the updater verifies a signed manifest and immutable download before it examines the ZIP. It copies the download once into a private same-volume pending archive and uses that exact staged file for validation, extraction, and publication. The staged regular file's identity, single-link state, exact size, and SHA-256 are checked before and after extraction and immediately before the Keychain rollback floor is persisted and the archive is atomically renamed into place. Before extraction, it cross-checks central/local entry names, flags, methods, CRC/sizes, and strict non-ZIP64 data descriptors, rejects overlapping records, and streams every stored/raw-deflate payload to prove its actual expanded size and CRC-32. It rejects encrypted, split, ZIP64, unsupported-compression, traversal, absolute, ambiguous or colliding paths; symbolic links and other special files; duplicate local offsets; and any record that reaches or overlaps the central directory. A post-extraction walk repeats the count, type, and size checks and rejects symbolic links, hard links, and other special files. The publication validator independently reconstructs the canonical envelope payload and verifies its P-256 signature before accepting the selected key and validity window.
...
- The Local Console launch URL contains a fragment nonce only until bootstrap; browser developer tools show protected requests using an `Authorization: Bearer …` header and no Scopeproof localhost authentication cookie.
- **Open Evidence Folder** opens the active identity's evidence root: `~/Documents/Scopeproof Evidence` for `local/default`, or the matching `tenants/<tenant>/workspaces/<workspace>/` subtree for a named identity.
...
| Browser reports unauthorized | Do not reuse or share an old loopback URL; reopen the console from the shield menu to establish a fresh fragment nonce and in-memory bearer. Switching tenant/workspace intentionally invalidates the former session. |
...
| Hosted upload reports scanner or timestamp unavailable | Keep using the local artifact, ask the service operator to restore the independent scanner/RFC 3161 boundary, then retry the unchanged schema-8 pair. The server intentionally stores nothing for this failed attempt. |
