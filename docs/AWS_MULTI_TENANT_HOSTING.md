...
`jsontechology.com` is an illustrative planning placeholder, not checked-in infrastructure configuration or a claim of ownership. The CDK `rootDomain` and `deploymentEnvironment` defaults are deliberately empty; every run must also supply the complete tenant list and an explicit recovery object. Example customer hostnames are:
...
Route 53 Alias -> regional API Gateway + WAF
        ...
        +---- per-tenant data and legal-control Lambdas
              (/v1/me, /v1/upload-intents, /v1/evidence/search,
               /v1/evidence-download-intents, /v1/legal-hold-*)
                    |
...
The 2026-09-01 source remediation adds route-specific API Gateway Cognito
scopes, production-required exact-version DLP, durable KMS-signed DLP receipts,
and DLP/promotion-fact reconciliation across S3, DynamoDB, and PostgreSQL. It
does not change the deployment boundary below: no AWS resource was deployed or
live-validated. The complete source-versus-operations matrix is in
[Security and product remediation status — 2026-09-01](SECURITY_REMEDIATION_2026-09-01.md).
...
| --- | --- | --- |
| AWS infrastructure | Synthable shared and per-tenant CDK stacks, DynamoDB global control table, explicit tenant resources, Cognito clients, WAF, budgets, alarms, release distribution, provisioning workflow, quarantine scanning resources, retained audit storage, and a gated bounded upload-lifecycle reconciler whose rule and SQS mapping default disabled until the exact backfill is verified. Maintenance uses a protected tenant directory, a new base-PK sparse V2 index, per-tenant budgets, seven-day event grace, buffered TTL, durable action-required ledger/resolution, and provider-error retry through SQS; legacy GSI1 is retained and dual-written for staged rollback. | Controlled account deployment, safe migration of any existing single-region control table, verified additive V2 creation/backfill before worker activation, delayed-event/tenant-padding/action-ledger/DLQ canaries, verified domain and certificates, real alert recipients, UI source/release connection, integration tests, restore drills, and penetration testing. |
| Tenant request boundary | Strongly consistent exact-host resolution, strict Cognito RS256/JWKS access-token verification, exact web/native app-client allowlist, operation OAuth scopes, RDS Data active-membership/RBAC lookup, safe API errors, an API Gateway mock `GET /health`, and authenticated upload, Aurora-backed metadata search, 60-second exact-version download, and two-person legal-hold routes. Upload issuance atomically limits each member/tenant to 60/300 requests per minute and 500/5,000 new reservations per UTC day. | Deploy it, connect the approved customer UI and native OAuth callback/session controller, validate quota/pagination/download/replay behavior under real concurrency, migrate the remaining product routes, and prove the boundary against two tenants. |
| Relational data | PostgreSQL baseline, tenant-aware keys, forced RLS, six separate upload/ingest/evidence-control-worker/legal-hold-API/evidence-read/API-audit-signer logins, execute-only procedure allowlists with no direct application table grants, an offline renderer, and an AWS provisioning implementation. | Port remaining D1 repositories/queries, run the provisioner in an isolated AWS account, verify all role boundaries and RLS with two tenants, test backup/restore, and reconcile imported production data. |
| Evidence storage | Per-tenant KMS, quarantine and Object-Locked buckets, server-managed tenant retention, a write-only/no-list upload IAM boundary, composed upload-intent route, Dynamo/Aurora ambiguous-commit reconciliation, production-required exact-version HTTPS DLP with a KMS-encrypted token secret, durable KMS-signed DLP and promotion receipts, S3/Dynamo/PostgreSQL fact reconciliation, and CDK-wired two-person `REQUESTED` → `APPROVED` → `APPLYING` → `APPLIED` exact-version S3 legal-hold routes plus terminal `REQUESTED` → `EXPIRED`, durable signed audit/recovery-publication outboxes, bounded retry/backoff, and age/failure/expiry alarms. The public legal-hold API can only request/approve in SQL; only the separate worker role can expire/reconcile and use S3/KMS/audit state. | Deploy/test the exact-version DLP and active clean/rejected paths, approve scanner privacy/retention/region/availability/efficacy, add the legal-hold UI/operating process, prove independent authorization and KMS audit emission in AWS, and complete replay/chaos/operational canaries before accepting evidence. |
| Recovery and native release | DynamoDB global control table, same-account cross-region S3 Object-Lock live replication, deterministic S3 Batch backfill, recurring 24-hour exact-version checksum/metadata/KMS/Object-Lock verification with 36-hour freshness alarms, Aurora AWS Backup/Vault Lock plus regional recovery-freshness alarms, a protected Developer ID/notarization workflow, and advanced CodeQL with a manual Swift build plus JavaScript/TypeScript/Actions coverage. GitHub default CodeQL setup was disabled on 2026-08-27 so the advanced workflow can upload results. | Safely migrate any existing control table, deploy bootstrap/enabled recovery phases, perform backfill/restore/RPO/RTO/cutover drills, configure protected environments and Apple credentials, require all language checks, confirm current advanced-analysis coverage, and explicitly publish an approved release. |
...
| Legacy hosted application | The repository retains source for a conditional single-tenant Cloudflare/Sites application. It may be used only in a separately authorized private deployment after the identity-aware dispatcher, header replacement, canonical-origin, and exclusive-ingress controls are proven; this repository does not establish that such a deployment exists. | Convert the vinext/Worker runtime and D1/R2 adapters to the AWS tenant-aware implementation, migrate and reconcile one legacy tenant, then pass Client A/Client B isolation gates before adding another customer. |
...
4. Extend the strict Cognito/exact-host/scope/membership boundary from the
   implemented identity, upload, evidence-read, and legal-hold routes to every
   migrated API; add membership administration and
