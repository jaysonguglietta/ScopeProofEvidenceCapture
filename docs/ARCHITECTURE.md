...
1. **User and browser → private Sites application:** Sites supplies authenticated identity headers. APIs reject requests without identity and apply server-side role checks.
...
6. Approved evidence can be decrypted transiently for a bounded export, re-hashed, indexed, signed with ECDSA P-256, encrypted again for R2 storage, and made available through an expiring download.
