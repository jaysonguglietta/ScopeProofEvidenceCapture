...
| Local Console / SQLite | A `127.0.0.1`-only authenticated browser UI and rebuildable metadata index that presents local screenshots with S3 inventory for search and grouping. It joins locations only through an exact local upload receipt; S3-only pairs remain provenance-unverified. |
...
5. **Browser → native Local Console:** an ephemeral launch URL establishes an HttpOnly SameSite session. The server binds only to `127.0.0.1`, validates the exact Host/Origin/Fetch Metadata boundary, accepts no filesystem paths or S3 keys, and resolves artifacts exclusively by validated evidence ID. The server—not JavaScript—reads Keychain credentials and the verified S3 destination. A local/S3 join requires a schema-2 receipt binding exact keys, versions, ETags, checksums, account, settings, and local manifest digest. An S3 preview first validates the paired exact-version manifest, then the exact PNG digest, is limited to 40 MiB, and is removed from private temporary storage. A self-consistent S3-only pair remains unverified provenance.
