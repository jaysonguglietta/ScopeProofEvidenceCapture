...
1. Apply and verify every database migration through `0023_independent_image_safety.sql`. In particular, `0020_native_device_chain.sql` adds the device-chain state, `0022_native_provenance_quarantine.sql` adds immutable schema-7 provenance/finalization fields, and `0023_independent_image_safety.sql` adds the exact-pixel server-scan binding.
...
5. Exercise one current schema-7 upload end to end. Verify the server checks the device P-256 signature and exact digests, independently scans the exact PNG, verifies the RFC 3161 result, reserves the next contiguous device sequence, stores the artifact, and finalizes the artifact/digest/event/key linkage.

The read, preview, approval, package/export, and Jira disclosure paths must continue to fail closed when the independent scan binding or final native-chain link is missing. Scanner or required-timestamp failure before storage should return a recoverable error and store nothing. A cross-store failure after storage is a quarantine condition: preserve the record, block disclosure, and retry the original exact schema-7 upload to reconcile it. Unsigned legacy evidence must be recaptured; never backfill trust fields, change status directly, or grandfather a population with a blanket SQL update.
...
- Migrations are applied through `0023`; independent screenshot scanning is configured through the legacy-named `BROWSER_OCR_*` settings; recognized OCR text is not retained; and the required RFC 3161 trust boundary is configured and tested. Production readiness fails when either service is disabled, missing, unsafe, or unverifiable.
...
- A representative current Mac completed schema-7 signed capture → independent exact-pixel server scan → trusted RFC 3161 timestamp → final server chain linkage → independent review → scoped export → Jira disclosure test without manual database repair. Legacy and deliberately unbound fixtures remained quarantined.
