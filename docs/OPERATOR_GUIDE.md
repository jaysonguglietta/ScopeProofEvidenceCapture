...
Until every hosted step succeeds, the item is unverified/quarantined and cannot be read, previewed, approved, packaged, exported, or sent to Jira. An unsigned legacy capture cannot be converted into trusted evidence by changing a status or database field. Recapture it with the current app. If an otherwise current upload is missing its server scan or final chain link, retry the original exact schema-7 PNG/manifest pair so the server can rescan and reconcile it.
...
| Upload remains pending | Verify that the HTTPS server origin is compiled into this release and exactly matches the entered URL, then verify the active device token. If the service reports scanner or RFC 3161 unavailability, preserve the local artifact and retry the unchanged schema-7 pair after recovery. |
