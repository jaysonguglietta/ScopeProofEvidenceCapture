...
**Review target:** current working tree, including the legacy Cloudflare/Sites runtime, the native macOS application, the AWS multi-tenant foundation, database migrations, CI/release automation, and operator documentation
...
| SP-05 | Native evidence could become trust-bearing before its device-chain link finalized | Medium | High | Remediated in source; orphan reconciliation/fault testing remains |
...
### Material remediations verified in the current working tree
...
- Hosted schema-7 manifest parsing accepts native source provenance only as an exact HTTP(S) origin matching `sourceHost`; paths, query strings, fragments, credentials, ports, and host substitution are rejected server-side to prevent URL-secret persistence.
- Hosted native upload now accepts only schema 7, verifies P-256 provenance, pins one signing key to one device, enforces exact session ownership/scope, and advances a server-maintained device chain with a lease/CAS protocol.
...
7. **Mac → legacy hosted API.** The device token and client can be stolen or malicious. Schema-7 P-256 signatures, key pinning, session checks, and server chain CAS authenticate device origin/continuity, but they do not prove honest local OCR, uncompromised execution, or successful completion of the distributed persistence workflow. The server therefore treats the local safety claim as advisory and independently scans the exact PNG before storage.
...
| Membership/devices | invites, roles, status, bearer device token, app version, signing key | invitation-only after bootstrap, final-admin trigger, permanent revoke, token hash/expiry/rotation, owner status recheck, schema-7 P-256 pinning and chain CAS | compromised endpoint can still use its legitimate identity; no hardware/MDM attestation |
| Evidence upload/read | multipart, PNG, JSON/text, metadata, artifact ID, inline/download | content/type/size validation, PNG parser, redaction, AES-GCM, digest, inert response headers, authorization, audit-before-disclosure, exact pre-storage screenshot scan/receipt, all-screenshot read/approval/export gates, pending native provenance fails closed | scanner trust/privacy/availability and OCR false negatives; pending native orphan cleanup needs operations |
...
| Native filesystem | PNG/JSON/sidecars, roots, symlinks/hardlinks | validated central loader, containment, `O_NOFOLLOW`, bounds, digests, signed schema-7 provenance | local Keychain anchor is not an independent transparency log |
| Local Console | loopback HTTP, Host, Origin, cookie, IDs, concurrency | `127.0.0.1`, launch nonce, HttpOnly/SameSite session, expiry, connection/time limits, server-side path/key resolution | endpoint compromise still defeats local trust; never expose to LAN |
...
- **Severity:** Medium
- **Confidence:** High
- **Classification/status:** Confirmed pre-fix transactional/provenance gap; **remediated in the current source**. Orphan-object cleanup and deployed fault injection remain residual validation.
- **Affected files/functions:** `app/api/native/evidence/route.ts` (`POST`), `lib/server/devices.ts` (`reserveCaptureDeviceChain`, `finalizeCaptureDeviceChain`), `lib/server/evidence.ts` (`storeEvidence`), review/export eligibility queries
...
- **Potential impact:** The pre-fix impact was incomplete chain-of-custody presented as reviewable evidence. Current residual impact is availability/storage: pending rows or R2 objects can require retry/reconciliation and may accumulate after repeated failures, but they cannot enter the trust-bearing workflows reviewed here.
- **Recommended fix:** Preserve every finalized-link predicate and migration 0022 immutability constraint. Add a scheduled, bounded reconciliation/cleanup job for expired leases and pending artifacts, with an audited terminal quarantine state rather than silent deletion where retention policy requires preservation.
- **Example secure implementation / patch guidance:** Reconciliation should load the exact reserved tuple `(device_id, local_evidence_id, sequence, manifest_sha256, image_sha256, event_hash, provenance_key_id)`, idempotently call the same guarded finalizer when all facts match, and otherwise quarantine the artifact and object for operator review. It must never advance a sequence from metadata alone or skip a missing predecessor.
...
| Local Console session and resource controls were weak | High / High | `LocalConsoleServer.swift`, `LocalConsoleAssets.swift` | Malicious local/web origins could reuse launch state or exhaust loopback connections. | Per-launch single-use nonce, rotated HttpOnly session, idle/absolute expiry, 32-connection cap, 15-second deadline, strict Host/Origin/fetch metadata, ID-only resolution. Test DNS rebinding and concurrent slow clients. | CWE-346, CWE-400, CWE-613 |
...
2. It submits secret-bearing PNG pixels with a valid schema-7 signature, correct chain event, and false passing local-safety declaration.
...
1. A valid schema-7 upload reserves the exact next device-chain event.
...
**Combined risk:** The trust-boundary weakness in SP-05 is remediated: partial persistence fails closed instead of becoming evidence. Repeated faults can still consume encrypted storage and operator attention, so bounded orphan reconciliation and deployed failure injection remain necessary availability controls.
...
- **Native pending-state cleanup is operationally incomplete.** The source now labels and fails closed on pending provenance across trust-bearing paths, but a bounded scheduled reconciler/quarantine workflow is still needed to clean up artifacts left by repeated cross-store failures.
...
4. Add bounded native orphan reconciliation/cleanup and prove the implemented pending-state fail-closed selectors with deployed D1/R2 fault injection.
...
7. Preserve the package occurrence/revision fence, schema-7 signature/key/session/chain checks, migration 0022 immutable native links, migration 0023 universal screenshot receipts, immutable signed witness receipts, and all-key readiness self-tests as release-blocking regressions.
...
- Verify schema-6 remains browse-only and current signed evidence is required for upload, review, Jira, hold, retention deletion, S3 receipt binding, and export.
...
- Test Local Console DNS rebinding, wrong Host/Origin/fetch metadata, stolen/expired cookie, replayed launch nonce, slowloris, excessive connections, oversized headers/body, and arbitrary ID/path/key input.
