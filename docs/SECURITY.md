...
> The current hosted runtime remains single-tenant. The future AWS production boundary and its required tenant-isolation tests are documented in [AWS multi-tenant hosting](AWS_MULTI_TENANT_HOSTING.md); confirmed gaps and release blockers are tracked in the [AWS adversarial security review](AWS_SECURITY_REVIEW.md). Authentication or a Route 53 subdomain alone does not make the existing global data paths safe for unrelated customers.
