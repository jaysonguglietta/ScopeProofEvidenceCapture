...
export function configuredMacRelease(env: ScopeproofEnv, now = new Date()): { manifest: MacReleaseManifest; signatureDERBase64: string } {
  ...
  const expectedPath = `/macos/${manifest.version}/Scopeproof-Capture-${manifest.version}.zip`;
  if (!allowedHosts.size || allowedHosts.size > 10 || url.protocol !== "https:" || url.username || url.password || url.port || url.search || url.hash || url.pathname !== expectedPath || !allowedHosts.has(url.hostname.toLowerCase())) throw new Error("macOS release download host or immutable path is not approved.");
